...
not just the percentage — e.g. "every change to `attach_error` also touches
`sites_controller.rb` because the after_action is registered there, so the two are a unit." That
