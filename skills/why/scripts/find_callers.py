...
For --file:
  Searches in order of specificity, then merges hits:
    1. The path-derived **namespaced symbol** (e.g. lib/netlify_server/client.rb →
       `NetlifyServer::Client` for Ruby) — most precise, lowest noise.
...
def derive_namespace(file_path: str):
    """Path → namespaced Ruby-style symbol, e.g. lib/netlify_server/client.rb → NetlifyServer::Client.
...
def main():
    ...
    if args.symbol:
        ...
        if ns and "::" in ns:
            candidates.append(ns)             # 1. NetlifyServer::Client (most specific)
    ...
    # `NetlifyServer::Client` in unrelated noise.
