...
- **Linear.** IDs match `[A-Z][A-Z0-9]+-\d+` (e.g. `ABC-1234`) or appear as `linear.app/...`
...
`lib/payment_gateway/client.rb` → `PaymentGateway::Client`) over the bare basename — this
avoids drowning the result in unrelated matches for generic stems like `client`, `service`,
or `handler`. If the namespaced form has any hits, only those are returned; otherwise it
falls through to CamelCase and snake_case basename matches.
...
```json
{
  ...
  "frequent_authors": [
    {"name":"Jane Doe","email":"jane@example.com","commits":44,
  ...
  "discussions": [
    {
      ...
      "title":"<short heading: e.g. 'jdoe review on PR #1234', '#design-channel discussion', 'PROJ-101 ticket intent'>",
...
| Linear ticket | `[ABC-1234](<url from get_issue>)` — use the canonical `url` the Linear MCP returns |
